...
public:
  ...
  {
    assert(bucket_num > 0);
    index_size = sizeof(Directory) + MAX_SEG * sizeof(Segment) + sizeof(Pair) * MAX_KV + sizeof(TimeCount) + sizeof(SplitArray) + MAX_SEG * BUCKET_NUM * SLOT_NUM * sizeof(partial_t); // size_t use signal
    ...
    split_ptr = count_ptr + sizeof(TimeCount);
    partial_ptr = split_ptr + sizeof(SplitArray);
    ...
    memset(partial_ptr, 0, MAX_SEG * BUCKET_NUM * SLOT_NUM * sizeof(partial_t));
  }
