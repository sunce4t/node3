...
public:
  ...
  bool RDMAWriteUnsignal(coro_id_t coro_id, RCQP *qp, char *wt_data, uint64_t remote_offset, size_t size);
  ...
  bool RDMACASUnsignal(coro_id_t coro_id, RCQP *qp, char *local_buf, uint64_t remote_offset, uint64_t compare, uint64_t swap);
...
ALWAYS_INLINE
bool CoroutineScheduler::RDMAWriteUnsignal(coro_id_t coro_id, RCQP *qp, char *wt_data, uint64_t remote_offset, size_t size)
{
  auto rc = qp->post_send(IBV_WR_RDMA_WRITE, wt_data, size, remote_offset, 0, coro_id);
  if (rc != SUCC)
  {
    RDMA_LOG(ERROR) << "client: post write fail. rc=" << rc << ", tid = " << t_id << ", coroid = " << coro_id;
    return false;
  }
  // AddPendingQP(coro_id, qp);   //don't append
  return true;
}
...
bool CoroutineScheduler::RDMARead(coro_id_t coro_id, RCQP *qp, char *rd_data, uint64_t remote_offset, size_t size)
{
  ...
  if (remote_offset > 48UL * 1024 * 1024 * 1024)
    std::cout << "size! offset: " << remote_offset << " id: " << coro_id << std::endl;
  if (rc != SUCC)
  {
...
ALWAYS_INLINE
bool CoroutineScheduler::RDMACASUnsignal(coro_id_t coro_id, RCQP *qp, char *local_buf, uint64_t remote_offset, uint64_t compare, uint64_t swap)
{
  auto rc = qp->post_cas(local_buf, remote_offset, compare, swap, 0, coro_id);
  if (rc != SUCC)
  {
    RDMA_LOG(ERROR) << "client: post cas fail. rc=" << rc << ", tid = " << t_id << ", coroid = " << coro_id;
    return false;
