                        ...
                        std::list<HashRead> &pending_next_hash_ro)
{
  ...
  for (auto &res : pending_direct_ro)
  {
      ...
      if (likely(fetched_item->valid))
      {
        *it = *fetched_item;
        ...
        if (unlikely((it->lock & STATE_INVISIBLE)))
        {
          ...
          if (!coro_sched->RDMARead(coro_id, res.qp, cas_buf, lock_offset, sizeof(lock_t)))
            return false;
        }
#endif
      }
    ...
    else
    {
      ...
      if (!coro_sched->RDMARead(coro_id, res.qp, (char *)local_hash_node, node_off, sizeof(HashNode)))
        return false;
    }
  }
  return true;
...
bool DTX::CheckInvisibleRO(std::list<InvisibleRead> &pending_invisible_ro)
{
  ...
  for (auto iter = pending_invisible_ro.begin(); iter != pending_invisible_ro.end();)
  {
    ...
    {
      // RDMA_LOG(DBG) << "LOCK VALUE IS: " << std::hex << lock_value;
      if (!coro_sched->RDMARead(coro_id, res.qp, res.buf, res.off, sizeof(lock_t)))
        return false;
      iter++;
    }
    else
