...
// select_backup = (select_backup + 1) % remote_backup_nodes->size();  // Load balance on backups
    ...
    RCQP *qp = thread_qp_man->GetRemoteDataQPWithNodeID(remote_node_id); // 通过该thread_qp_man来获取相应qp
    ...
    if (offset != NOT_FOUND)
    {
      ...
      char *data_buf = thread_rdma_buffer_alloc->Alloc(DataItemSize);
      pending_direct_ro.emplace_back(DirectRead{.qp = qp, .item = &item, .buf = data_buf, .remote_node = remote_node_id}); // 加入到read_only的队列中
