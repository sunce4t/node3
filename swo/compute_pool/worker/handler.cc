...
void Handler::GenThreads(std::string bench_name)
{
  ...
  Key_t *r_key;
  Value_t *r_value;
  if (run == load && RUN_NUM <= LOAD_NUM)
  {
    r_method = l_method;
    r_key = l_key;
    r_value = l_value;
    RDMA_LOG(INFO) << "Same Depth Use Same Array!";
  }
  else
  {
    r_method = (char *)malloc(sizeof(char) * RUN_NUM);
    r_key = (Key_t *)malloc(sizeof(Key_t *) * RUN_NUM);
    r_value = (Value_t *)malloc(sizeof(Value_t *) * RUN_NUM);
  ...
  {
    u_key[i] = (Key_t)calloc(KEY_LENGTH, sizeof(char));
    u_value[i] = (Value_t)calloc(VALUE_LENGTH, sizeof(char));
  }
  l_ifs.open(update);
  if (!l_ifs)
  {
  ...
  else
  {
    for (int i = 0; i < UPDATE_NUM; i++)
    {
      ...
      l_ifs.getline(u_value[i], VALUE_LENGTH);
    }
    l_ifs.close();
  ...
  for (t_id_t i = 0; i < thread_num_per_machine; i++)
  {
    ...
    param_arr[i].update_size = 0;
    param_arr[i].cas_size = 0;
    param_arr[i].write_size = 0;
    param_arr[i].read_size = 0;
    param_arr[i].finish_num = 0;
    param_arr[i].cache_dir = 0;
    param_arr[i].run_cnt = 1;
    param_arr[i].partial_size = 0;
    param_arr[i].full_size = 0;
    param_arr[i].total_cnt = 0;
    param_arr[i].hit_cnt = 0;
  ...
  #ifdef WARMUP
  chunk_size = WARM_NUM / thread_num_per_machine;
  for (t_id_t i = 0; i < thread_num_per_machine; i++)
  {
  ...
  //monitor real-time space used for cache
  // std::thread t = std::thread(&get_real_space_info,
                              // param_arr,
                              // &monitor);

  //monitor predictor 
  std::thread t = std::thread(&get_predictor_info,
                              param_arr,
                              &monitor);
#endif
  ...
  chunk_size = LOAD_NUM / thread_num_per_machine;
  for (t_id_t i = 0; i < thread_num_per_machine; i++)
  {
    if (i != thread_num_per_machine - 1)
    {
  ...
  // if (machine_num >= 1)
  // {
  //   RDMA_LOG(INFO) << "Type Any to Start";
  //   char _;
  //   scanf("%c", &_);
  // }
...
// #endif
  ...
  uint64_t total_cas_size= 0 ;
  uint64_t total_read_size = 0;
  uint64_t total_write_size = 0;
  for(int i = 0; i < thread_num_per_machine; i ++)
  {
    total_update_size += param_arr[i].update_size;
    total_cas_size += param_arr[i].cas_size;
    total_read_size += param_arr[i].read_size;
    total_write_size += param_arr[i].write_size;
  }
  double msr_sec = (msr_end.tv_sec - msr_start.tv_sec) + (double)(msr_end.tv_nsec - msr_start.tv_nsec) / 1000000000;
  std::cout << "update" << "\t" << "cas" << "\t" << "read" << "\t" << "write" << std::endl;
  std::cout << total_update_size << "\t" << total_cas_size << "\t" << total_read_size << "\t" << total_write_size << std::endl;
  std::cout << "Insert Speed: " << LOAD_NUM / msr_sec << "\t" << std::endl;
  signal = 0;
  connected_t_num = 0; // reset
  ...
  std::cout << "Search Speed: " << RUN_NUM / msr_sec << "\t" << std::endl;
#ifdef TEST_LATENCY
...
chunk_size = UPDATE_NUM / thread_num_per_machine;
  ...
  clock_gettime(CLOCK_REALTIME, &msr_end);
  msr_sec = (msr_end.tv_sec - msr_start.tv_sec) + (double)(msr_end.tv_nsec - msr_start.tv_nsec) / 1000000000;
  std::cout << "Update Speed: " << UPDATE_NUM / msr_sec << "\t" << std::endl;
  signal = 0;
  connected_t_num = 0; // reset
#endif
  ...
  clock_gettime(CLOCK_REALTIME, &msr_end);
  msr_sec = (msr_end.tv_sec - msr_start.tv_sec) + (double)(msr_end.tv_nsec - msr_start.tv_nsec) / 1000000000;
  std::cout << "Delete Speed: " << DELETE_NUM / msr_sec << "\t" << std::endl;
  signal = 0;
  connected_t_num = 0; // reset
#endif
