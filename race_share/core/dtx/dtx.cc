...
#if RFLUSH == 0
  if (!IssueCommitAll(pending_commit_write, cas_buf))
    return false;
#elif RFLUSH == 1
  if (!IssueCommitAllFullFlush(pending_commit_write, cas_buf))
    return false;
#elif RFLUSH == 2
  if (!IssueCommitAllSelectFlush(pending_commit_write, cas_buf))
    return false;
#endif
