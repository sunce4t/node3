...
#define SLOT_NUM 7
#define BUCKET_NUM 48 //(128 + 128 / 2) // 加上共享bucket
...
#define MAX_SEG 8388608// 2^20 // 262144 // 2^18 // 1048576 // 65536 1048576分配有问题
#define MAX_DEPTH 23
#define MAX_NODE 8
#define MAX_UNIT_NUM 8 // 每个线程的读/写操作注册在网卡地址的最大数量
#define UNIT_LEN 1536  // 与KV_LENGTH 与KEY_LENGTH挂钩 8个桶左右 会用到读取kv，读取一个kv对 1536 = 192 * 8
...
{
    uint8_t segment[6];
    depth_t local_depth; // 高位
} SegUnit Aligned8;

typedef struct Directory
{
    depth_t global_depth;  // 改!
    char emtpy[6];         // 占位
    uint64_t seg[MAX_SEG]; // 用于部分读取
    lock_t dir_lock;

    // lock_t seg_lock[MAX_SEG];
} Directory Aligned8;


// typedef struct LocalDirMeta
// {

//     std::shared_mutex seg_lock[MAX_SEG];
