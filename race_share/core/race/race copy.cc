...
bool RACE::UpdateDir(coro_yield_t &yield, bool sync_flag = false) /// TODO return false then yield
{
    ...
    if(global_dir_lock->try_lock() == false)
    {
        while(*local_dir_lock == 0);
        while(*local_dir_lock != 0)
        {
...
RE_SPLIT:
    if (GetDepth(cache_dir->seg[seg_num]) == cache_dir->global_depth) // this time cache_dir old
    {
        ...
        *local_dir_lock = coro_id;
        ...
        cout << "dir split:" << seg_num << "\tcoro_id:" << coro_id << "\tthread_id:" << t_id << endl;
        ...
        *local_dir_lock = coro_id;
        ...
        for (int i = 0; i < pow(2, cache_dir->global_depth - 1); i++) // other thread split
        {
            cache_dir->seg[i + (int)pow(2, cache_dir->global_depth - 1)] = cache_dir->seg[i]; // 将新分裂出来的目录的指针指向之前的段
        }
        // write new seg
        char *dir_buf = thread_rdma_buffer_alloc->Alloc(sizeof(SegUnit) * pow(2, cache_dir->global_depth - 1));
        memcpy(dir_buf, &cache_dir->seg[(int)pow(2, cache_dir->global_depth - 1)], sizeof(SegUnit) * pow(2, cache_dir->global_depth - 1));
        coro_sched->RDMAWrite(coro_id, qp, dir_buf, dir_offset + sizeof(uint64_t) + sizeof(SegUnit) * pow(2, cache_dir->global_depth - 1), sizeof(SegUnit) * pow(2, cache_dir->global_depth - 1));
        coro_sched->Yield(yield, coro_id);

        // cas change depth
        coro_sched->RDMACAS(coro_id, qp, lock_buf, dir_offset, (size_t)cache_dir->global_depth - 1, (size_t)cache_dir->global_depth); // release lock
        coro_sched->Yield(yield, coro_id);
