...
RE_UPDATE:
    ...
    if(sync_flag)
        before_gd = cur_gd;
    size_t NowSize = sizeof(uint64_t) + (1 << before_gd) * SegUnitSize; // get depth_t
...
bool RACE::UpdateRestDir(coro_yield_t &yield, depth_t now_depth)
{
    ...
    *local_dir_lock = coro_id; // lock it for cor
    ...
    {
        size_t NowSize = ((1 << now_depth) - (1 << cache_dir->global_depth)) * sizeof(uint64_t); // get depth_t
        char *data_buf = thread_rdma_buffer_alloc->Alloc(NowSize);
        // cout << "init success" << t_id << " coro_id: " << coro_id << endl;
        coro_sched->RDMARead(coro_id, qp, data_buf, global_meta_man->dir_offset + sizeof(uint64_t) + (1 << cache_dir->global_depth) * sizeof(uint64_t), NowSize);
...
bool RACE::UpdateSegUnit(coro_yield_t &yield, uint64_t seg_idx, uint64_t act_seg_idx, RCQP *qp)
{
    uint64_t offset = dir_offset + sizeof(uint64_t) + seg_idx * sizeof(uint64_t);
    char *seg_buf = thread_rdma_buffer_alloc->Alloc(SegUnitSize);
    coro_sched->RDMACAS(coro_id, qp, seg_buf, offset, cache_dir->seg[seg_idx], cache_dir->seg[act_seg_idx]);
    coro_sched->Yield(yield, coro_id);
    // uint64_t cmp = 0;
    CAS(&cache_dir->seg[seg_idx], &cache_dir->seg[seg_idx], cache_dir->seg[act_seg_idx]);
    // cache_dir->seg[seg_idx] = cache_dir->seg[act_seg_idx];
    return true;
}
...
bool RACE::Re_read(coro_yield_t &yield, Bucket *buc, size_t hash, size_t *seg_idx, uint64_t *offset, RCQP *qp) // async  re read bucket
{
    ...
    *seg_idx = hash & ((1 << cache_dir->global_depth) - 1);
    // if(cache_dir->seg[*seg_idx] == 0)
    ...
    for (int i = 0; i < SLOT_NUM; i++) // 比较suffix开始
    {
        for (int j = 0; j < 2; j++)
        {
            if (f_buc[j].slot[i] == nullptr) // 能不能Batch?
            {
                ...
                // if (f_buc[0].local_depth != GetDepth(cache_dir->seg[f_seg_idx])) // 若正在分裂
                {
                    act_seg_idx = f_hash & ((1 << f_buc[0].local_depth) - 1);
                    if(GetDepth(cache_dir->seg[f_seg_idx]) != GetDepth(cache_dir->seg[act_seg_idx]))
                    {
                        UpdateSegUnit(yield, f_seg_idx, act_seg_idx, qp);
    ...
    for (int i = 0; i < SLOT_NUM; i++) // 比较suffix开始
    {
        for (int j = 0; j < 2; j++)
        {
            if (s_buc[j].slot[i] == nullptr) // 能不能Batch?
            {
                ...
                {
                    act_seg_idx = s_hash & ((1 << s_buc[0].local_depth) - 1);
                    if(GetDepth(cache_dir->seg[s_seg_idx]) != GetDepth(cache_dir->seg[act_seg_idx]))
                    {
                        UpdateSegUnit(yield, s_seg_idx, act_seg_idx, qp);
...
SplitStatus RACE::Split(coro_yield_t &yield, uint64_t seg_num) // only when other cor split dir return fasle;TODO Yiled if false
{
    ...
    if (*((lock_t *)lock_buf) != UNLOCK && *((lock_t *)lock_buf) != t_id) // 若获取锁失败，目标值为true，代表别的线程在更新该seg
    {
        ...
        coro_sched->RDMARead(coro_id, qp, lock_buf, dir_offset + sizeof(uint64_t) + seg_num * sizeof(SegUnit), sizeof(lock_t));
        coro_sched->Yield(yield, coro_id);
        if(*(uint64_t *)lock_buf != cache_dir->seg[seg_num])
            CAS(&cache_dir->seg[seg_num], &cache_dir->seg[seg_num], *(uint64_t *)lock_buf);
        // UpdateDir(yield);
...
RE_SPLIT:
    if (GetDepth(cache_dir->seg[seg_num]) == cache_dir->global_depth) // this time cache_dir old
    {
        ...
        // 本地的目录正在更新
        if (*local_dir_lock != 0 && *local_dir_lock != coro_id) // other cor may used update
        {
            coro_sched->YieldForUpdateDir(yield, coro_id); // yield for other thread
            goto RE_SPLIT;
        }
        ...
        *local_dir_lock = coro_id;
        ...
        uint64_t cmp = UNLOCK;
        if(CAS(global_split_lock, &cmp, t_id) == false && *global_split_lock != t_id)
        {
            while(*global_split_lock != UNLOCK)
            {
        ...
        *local_dir_lock = coro_id;
        ...
        uint64_t NowSize = sizeof(uint64_t) + SegUnitSize * (int)pow(2, cache_dir->global_depth);
        char *data = thread_rdma_buffer_alloc->Alloc(NowSize);
        coro_sched->RDMARead(coro_id, qp, data, global_meta_man->dir_offset, NowSize);
        coro_sched->Yield(yield, coro_id);
        uint64_t swap;
        for (int i = 0; i < pow(2, cache_dir->global_depth); i++) // other thread split
        {
            swap = *(uint64_t *)(data + SegUnitSize * (i + 1));
            if(cache_dir->seg[i] != swap)
                CAS(&cache_dir->seg[i], &cache_dir->seg[i], swap);
...
RE_POINT:
    ...
    coro_sched->RDMARead(coro_id, qp, lock_buf, seg_offset + act_depth * SegSize + sizeof(Bucket) * BUCKET_NUM, 8);
    coro_sched->Yield(yield, coro_id);
    if((*(uint64_t*)lock_buf) != UNLOCK)
        cout << *(uint64_t*)lock_buf << endl;
    coro_sched->RDMARead(coro_id, qp, lock_buf, seg_offset + new_locate * SegSize + sizeof(Bucket) * BUCKET_NUM, 8);
    coro_sched->Yield(yield, coro_id);
    if((*(uint64_t*)lock_buf) != UNLOCK)
        cout << *(uint64_t*)lock_buf << endl;
