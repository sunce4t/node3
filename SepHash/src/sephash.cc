                ...
                uint64_t _machine_id, uint64_t _cli_id, uint64_t _coro_id)
{
    ...
    // std::cout << "CurSegMeta " << sizeof(CurSegMeta) << std::endl;
    // std::cout << "CurSegSize: " << sizeof(CurSeg) << std::endl;
...
Client::~Client()
{
    //printf("client %d total & read kv : %d %d\n", cli_id, total_read_cnt, read_kv_cnt);
    // log_err("[%lu:%lu] miss_cnt:%lu", cli_id, coro_id, miss_cnt);
    if(coro_id == 0) {
        uint64_t write_traffic = conn->write_traffic + wo_wait_conn->write_traffic;
        uint64_t read_traffic = conn->read_traffic + wo_wait_conn->read_traffic;
        printf("write_traffic: %lf GB, read_traffic: %lf GB", conn->write_traffic*1.0/1024/1024/1024, conn->read_traffic*1.0/1024/1024/1024);
        printf(" Sync_traffic: %lf GB\n", sync_traffic*1.0/1024/1024/1024);
    }
...
task<uintptr_t> Client::check_gd(uint64_t segloc = -1,bool read_fp = false)
{
    if(segloc!=-1){
        ...
        sync_traffic += read_size + sizeof(uint64_t);
        co_await read_cur_ptr;
        co_return dir->segs[segloc].cur_seg_ptr;
    }
    co_await conn->read(seg_rmr.raddr, seg_rmr.rkey, &dir->global_depth, sizeof(uint64_t), lmr->lkey);
    sync_traffic += sizeof(uint64_t);
    co_return 0;    
}
...
Retry:
    ...
    sync_traffic += sizeof(CurSegMeta);
    // b. read slots
...
Retry:
    ...
    // log_err("len:%d",dir->segs[segloc].fp[pattern_fp1].num);
    ...
    // log_err("read at seg_loc:%lu with main_ptr:%lx",segloc,seg_meta->main_seg_ptr);
        ...
        for (uint64_t i = 0; i < end_pos - start_pos; i++)
        {
            ...
            if (main_seg[i] != 0 && main_seg[i].fp == pattern_fp1 && main_seg[i].dep == dep_info)
            {
                if(main_seg[i].fp_2 == pattern_fp2 || cnt == 1){
                    ...
                    read_kv_cnt ++;
                    // main_seg[i].print();
                    // kv_block->print_var_kv();
                    // log_err("cmp_res:%d",memcmp(key->data, kv_block->data, key->len));
