...
then
    ...
    --mem_size 182536110080 #91268055040
else
    echo "machine" $1

    ycsb_load="/home/dell/yzx/ycsb_output/$4-node/uniform/insert_delete_100M"
    ycsb_run="/home/dell/yzx/ycsb_output/$4-node/uniform/read_100M" 
    for num_cli in `seq $2 $2`;do
        for num_coro in `seq $3 $3`;do
            for load_num in 100000000;do
                ...
                ./ycsb_test \
                --server_ip 10.26.43.56 --num_machine $4 --num_cli $num_cli --num_coro $num_coro \
                --gid_idx 1 \
                --max_coro 256 --cq_size 64 \
                --machine_id $1  \
                --load_num $load_num \
